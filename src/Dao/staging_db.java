...
public class staging_db {
	...
	public void updateDataInTempTable() {
		String updateSizeSql = "UPDATE temp_product_daily " + "SET size = 'N/A' "
				+ "WHERE size IS NULL OR TRIM(size) = ''";

		String updateDiscountSql = "UPDATE temp_product_daily " + "SET discount = ABS(discount) "
				+ "WHERE discount < 0";
...
//        String filePath = ctdb.getLatestFilePath();
		String loadDataSql = "LOAD DATA INFILE 'C:/ProgramData/MySQL/MySQL Server 8.0/Uploads/products.csv' "
				+ "INTO TABLE staging_db.temp_product_daily " + "FIELDS TERMINATED BY ',' "
	...
	public void insertDataFromTempToMain() {
		String insertDataSql = "INSERT INTO product_daily (productID, productName, description, category, originalPrice, discountedPrice, discount, brand, size, rating, date) "
				...
				+ "STR_TO_DATE(temp_date, '%Y-%m-%d') AS date FROM temp_product_daily";

		try (Connection conn = getConnection(); Statement stmt = conn.createStatement()) {

			// Thực hiện câu lệnh SQL
			int rowsAffected = stmt.executeUpdate(insertDataSql);
			System.out.println("Số dòng đã được chèn: " + rowsAffected);
		} catch (SQLException | ClassNotFoundException e) {
			// In ra lỗi nếu có
